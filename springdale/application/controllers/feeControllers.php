...
class feeControllers extends CI_Controller{
	...
	function payFee(){
		...
		$feeSehedule["finance_start_date"] =$this->session->userdata("fsd");
		foreach($fields as $field):
			if($field != "id" && $field != "student_id" && $field != "finance_start_date"):
...
//-----------------------------------------------------------------------------------------------
	...
	function feeReport(){
		...
		function feeRemSms(){
			$info=$this->session->userdata("your_school_name");
			$sdue=$this->input->post("smstodue");
			$mnum=$this->input->post("mnum");
			$sname=$this->input->post("sname");
			$fname=$this->input->post("fname");
			$msg =	"Dear Parent ".$fname." your Ward's (".$sname.") school fee ".$sdue." is remain to deposit. Please deposit soon.Ignore message if already deposited ".$info;
			sms($mnum,$msg);
			echo "Success";

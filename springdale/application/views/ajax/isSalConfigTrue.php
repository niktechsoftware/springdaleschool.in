...
if($qres->num_rows() > 0){
$month = $a->row()->month;
$b=$this->db->query("SELECT fsd FROM emp_salary_info WHERE emp_id = '$enum' order by `id` asc limit 1")->row();
if($month>0){
												$fsd=$b->fsd;
												}else{
					...
					<div class="form-group">
						...
						<select multiple="multiple" id="sld" name="diposit_month[]" class="form-control search-select" required="required">
                            <?php for($i=$month; $i<=11; $i++):?>
                                <option id =id="sld" value="<?php echo date("M-y",strtotime("$fsd + $i month"));?>">
			...
			<div class="row">
				...
				<div class="col-md-3">
					...
					<div class="form-group">
						...
						$month1= $month + 4;
						if($month>=10){
						$edate="2016-".$month1."-31";
						$sdate="2016-".$month1."-01";
						}if($month<10){
						$edate="2016-0".$month1."-31";
						$sdate="2016-0".$month1."-01";
						}
